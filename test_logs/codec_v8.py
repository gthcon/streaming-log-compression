#!/usr/bin/env python3
"""
V8 Ultimate Log Codec - XOR Delta Focus

Key innovations over V7:
1. XOR delta encoding with length-based window (like LogLite)
2. RLE encoding for zero runs after XOR
3. Multi-space preprocessing for whitespace preservation
4. Frame-of-reference timestamp encoding
5. Path columnar encoding
6. Recursive smart column encoding for JSON
"""
...
# Multi-space preprocessing marker
MULTI_SPACE_MARKER = '«'

# XOR Window parameters
XOR_WINDOW_SIZE = 16  # Keep last N lines per bucket
XOR_BUCKET_SIZE = 32  # Group lines within bucket_size of each other
XOR_MATCH_THRESHOLD = 0.5  # Minimum match ratio (50%)
...
# ============================================================================
# XOR Delta Encoding (LogLite-style)
# ============================================================================

class XORWindow:
    """Window-based XOR encoding like LogLite with length bucketing."""
    def __init__(self, window_size=XOR_WINDOW_SIZE, bucket_size=XOR_BUCKET_SIZE):
        self.window_size = window_size
        self.bucket_size = bucket_size  # Group lines within bucket_size of each other
        # Map from bucket_id to list of (line_bytes, line_index, original_len)
        self.windows = defaultdict(list)
        self.line_count = 0

    def _get_bucket(self, line_len):
        """Get bucket ID for a line length."""
        return line_len // self.bucket_size

    def find_best_match(self, line_bytes):
        """Find the best matching line from window, with padding for different lengths."""
        line_len = len(line_bytes)
        bucket = self._get_bucket(line_len)

        # Check this bucket and adjacent buckets
        candidates = []
        for b in [bucket - 1, bucket, bucket + 1]:
            candidates.extend(self.windows.get(b, []))

        if not candidates:
            return None, -1, 0

        # Find candidate with most matching bytes
        best_match = None
        best_score = -1
        best_idx = -1
        best_padded_len = 0

        for stored_bytes, stored_idx, stored_len in candidates:
            # Pad shorter to match longer
            max_len = max(line_len, len(stored_bytes))
            padded_line = line_bytes + b'\x00' * (max_len - line_len)
            padded_stored = stored_bytes + b'\x00' * (max_len - len(stored_bytes))

            # Count matching bytes
            matches = sum(1 for a, b in zip(padded_line, padded_stored) if a == b)
            if matches > best_score:
                best_score = matches
                best_match = padded_stored
                best_idx = stored_idx
                best_padded_len = max_len

        # Only use XOR if at least threshold% bytes match after padding
        if best_score < best_padded_len * XOR_MATCH_THRESHOLD:
            return None, -1, 0

        return best_match, best_idx, best_padded_len
    ...
    def add_line(self, line_bytes, line_idx):
        ...
        window.append((line_bytes, line_idx, line_len))

        # Keep only last N entries per bucket
...
def rle_encode_zeros(data):
    """RLE encode with focus on zero runs."""
    result = bytearray()
    i = 0
    while i < len(data):
        if data[i] == 0:
...
def encode_prefix_delta(output, values, n_rows):
    """Encode prefix-ID patterns like 'user-123'."""
    prefix = None
    numbers = []
...
def encode_drain_column(output, values, n_rows):
    """Encode a column using Drain template mining."""
    if not HAS_DRAIN:
        encode_dict_column(output, values, n_rows)
        return
...
def encode_text_logs_xor(lines):
    """Encode text logs with XOR delta encoding (LogLite-style with padding)."""
    output = BytesIO()
    n_lines = len(lines)
    ...
    # XOR delta encoding
    output.write(bytes([TEXT_XOR_DELTA]))

    window = XORWindow()

    # For each line, store: (encoding_type, data)
    # encoding_type: 0 = raw, 1 = xor with reference
    encoded_lines = []

    for i, line in enumerate(preprocessed):
        line_bytes = line.encode('utf-8')

        # Find best match in window (with padding)
        match_bytes, match_idx, padded_len = window.find_best_match(line_bytes)

        if match_bytes is not None and padded_len > 0:
            # Pad current line to match
            padded_line = line_bytes + b'\x00' * (padded_len - len(line_bytes))

            # XOR encode
            xor_result = xor_bytes(padded_line, match_bytes)
            rle_result = rle_encode_zeros(xor_result)

            # Only use XOR if it's actually smaller (considering overhead)
            overhead = 4  # ref_offset + orig_len + padded_len + rle_len varints
            if len(rle_result) + overhead < len(line_bytes):
                encoded_lines.append((1, i - match_idx, rle_result, len(line_bytes), padded_len))
            else:
                encoded_lines.append((0, line_bytes, None, None, None))
        else:
            encoded_lines.append((0, line_bytes, None, None, None))
    ...
    for item in encoded_lines:
        ...
        if enc_type == 0:
            ...
            # XOR encoded
            ref_offset, rle_data, orig_len, padded_len = item[1], item[2], item[3], item[4]
            output.write(encode_varint(ref_offset))  # Reference offset
            output.write(encode_varint(orig_len))    # Original line length
            output.write(encode_varint(padded_len))  # Padded length for XOR
            output.write(encode_varint(len(rle_data)))
            output.write(rle_data)

    return output.getvalue()

def decode_text_logs_xor(data, pos):
    """Decode XOR delta encoded text logs."""
    n_lines, pos = decode_varint(data, pos)
    enc_mode = data[pos]
    pos += 1

    if enc_mode != TEXT_XOR_DELTA:
        raise ValueError(f"Expected TEXT_XOR_DELTA mode, got {enc_mode}")

    lines = []
    decoded_bytes = []  # Keep decoded bytes for XOR references (original, not padded)
    ...
    for i in range(n_lines):
        ...
        if enc_type == 0:
            ...
            line_bytes = data[pos:pos+line_len]
            pos += line_len
        else:
            # XOR encoded
            ref_offset, pos = decode_varint(data, pos)
            orig_len, pos = decode_varint(data, pos)
            padded_len, pos = decode_varint(data, pos)
            rle_len, pos = decode_varint(data, pos)
            rle_data = data[pos:pos+rle_len]
            pos += rle_len

            # Decode RLE
            xor_result = rle_decode_zeros(rle_data, padded_len)

            # Get reference and pad it
            ref_idx = i - ref_offset
            ref_bytes = decoded_bytes[ref_idx]
            padded_ref = ref_bytes + b'\x00' * (padded_len - len(ref_bytes))

            # XOR to get padded result, then strip padding
            padded_result = xor_bytes(xor_result, padded_ref)
            line_bytes = padded_result[:orig_len]

        decoded_bytes.append(line_bytes)
        lines.append(postprocess_line(line_bytes.decode('utf-8')))

    return lines

def encode_text_logs_drain(lines):
    """Encode text logs with Drain template mining."""
    output = BytesIO()
    n_lines = len(lines)
    output.write(encode_varint(n_lines))
...
def encode_text_logs(lines):
    """Encode text logs - XOR is almost always better after zstd compression."""
    # XOR encoding produces highly compressible output (lots of zeros)
    # Drain produces smaller pre-compression but worse post-compression
    # Since we apply zstd afterward, use XOR
    return encode_text_logs_xor(lines)

def decode_text_logs(data, pos):
    """Decode text logs."""
    # Peek at encoding mode
    n_lines, next_pos = decode_varint(data, pos)
    enc_mode = data[next_pos]

    if enc_mode == TEXT_XOR_DELTA:
        return decode_text_logs_xor(data, pos)
    else:
...
def verify_file(input_path):
    ...
    print(f"V8 Processing {input_path}...")
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: codec_v8.py <logfile>")
        sys.exit(1)
